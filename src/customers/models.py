...
class Customer(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE) # 1 stripe customer per one user hence one-to-one
    ...
    def save(self, *args, **kwargs):
        ...
        # self.save() - this will cause recursion and fail after memory runs out


def allauth_user_signed_up_handler(request, user, *args, **kwargs): # receiver function for the signals
    email = user.email
...
def allauth_email_confirmed_handler(request, email_address, *args, **kwargs):
    ...
    for obj in qs: # triggers the save function
